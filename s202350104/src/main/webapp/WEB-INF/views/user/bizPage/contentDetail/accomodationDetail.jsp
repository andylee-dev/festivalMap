		    ...
		    color: #9BDB04;
				...
				<div class="container my-5" id="detail-body-container">
					...
					<div>
						...
						<input type="hidden" id="accomo_status"
					...
					<div class="my-5">
						<div class="" id="detail-main-container">
							<div class="container d-flex justify-content-around"
								...
								<label id="detail-top-text">숙박 ㅣ </label> <label
									id="detail-top-text">${accomodation.content_id} ㅣ</label>
								<c:choose>
							...
							<div class="container p-5" id="form-container">
								<form action="accomodation/update" method="post">
									<div class="mb-3">
										<label for="content_id" class="form-label">컨텐츠 ID</label> <input
											type="text" class="form-control" id="content_id"
											value="${accomodation.content_id} " readonly>
									</div>
									<div class="mb-3" id="detail-content-title">
										<label for="small_code" class="form-label">숙박 종류</label> <select
											class="form-select" aria-label="small_code">
											<c:forEach var="smallCode" items="${listSmallCode}">
												<c:if test="${smallCode.small_code != 999}">
													<option value="${smallCode.small_code}"
														${smallCode.small_code == accomodation.small_code? 'selected' : '' }
														disabled>${smallCode.content}</option>
												</c:if>
											</c:forEach>
										</select>
									</div>
									<div class="mb-3 ">
										<label for="title" class="form-label">숙소 이름</label> <input
											type="text" class="form-control" id="title"
											value="${accomodation.title} " readonly>
									</div>
									<div class="mb-3 ">
										...
										<div class="row">
											<div class="col-2">
												...
												<select class="form-select" id="area" name="area"
													onchange="getSigungu(this.value)" disabled>
													<option value="">전체</option>
													<c:forEach var="areas" items="${listAreas}">
														<c:if test="${areas.sigungu == 999}">
															<option value="${areas.area}"
																${areas.area == accomodation.area? 'selected':''}>${areas.content}</option>
														</c:if>
													</c:forEach>
												</select>
											</div>
											<div class="col-2">
												<!-- 두 번째 그리드 -->
												<select class="form-select" id="sigungu_select"
													name="sigungu" disabled>
													<option value="999">전체</option>
													<c:forEach var="areas" items="${listSigungu}">
														<c:if
															test="${areas.sigungu != 999 && areas.sigungu != null}">
															<option value="${areas.sigungu}"
																${areas.sigungu == accomodation.sigungu? 'selected':''}>${areas.content}</option>
														</c:if>
													</c:forEach>
												</select>
											</div>
											<div class="col-6">
												<input type="text" class="form-control" id="address"
													value="${accomodation.address} " readonly>
											</div>
										</div>
									</div>
									<div class="mb-3 ">
									...
									<div class="mb-3 ">
										<label for="email" class="form-label">email</label> <input
											...
											value="${accomodation.email} " readonly>
									</div>
									<div class="mb-3 ">
										<label for="phone" class="form-label">전화번호</label> <input
											type="text" class="form-control" id="phone"
											value="${accomodation.phone} " readonly>
									</div>
									<div class="mb-3 ">
										<label for="homepage" class="form-label">홈페이지</label> <input
											type="text" class="form-control" id="homepage"
											value="${accomodation.homepage} " readonly>
									</div>
									<div class="mb-3 ">
										<label for=created_at class="form-label">작성일</label> <input
											type="text" class="form-control" id="created_at"
											value="<fmt:formatDate value="${accomodation.created_at}" pattern="YYYY/MM/dd"/>"
											readonly>
									</div>
									<div class="mb-3 ">
										<label for="room_count" class="form-label">객실수</label> <input
											type="text" class="form-control" id="room_count"
											value="${accomodation.room_count} " readonly>
									</div>
									<div class="mb-3 ">
										<label for="reservation_url" class="form-label">예약처</label> <input
											type="text" class="form-control" id="reservation_url"
											value="${accomodation.reservation_url} " readonly>
									</div>
									<div class="mb-3 ">
										<label for="refund" class="form-label">환불규정</label> <input
											type="text" class="form-control" id="refund"
											value="${accomodation.refund} " readonly>
									</div>
									<div class="mb-3 ">
										<label for="check_in" class="form-label">입실시간</label> <input
											type="text" class="form-control" id="check_in"
											value="${accomodation.check_in} " readonly>
									</div>
									<div class="mb-3 ">
										<label for="check_out" class="form-label">퇴실시간</label> <input
											type="text" class="form-control" id="check_out"
											value="${accomodation.check_out} " readonly>
									</div>
									...
									<div class="col-12 d-flex justify-content-between">
										<div class="col-3 form-check mx-3">
											<input class="form-check-input" type="radio" name="is_credit"
												id="is_cook" value="1"
												${accomodation.is_cook == 1?"checked":""} disabled>
											<label class="form-check-label" for="is_credit">조리가능</label>
										</div>
										<div class="col-3 form-check mx-3">
											<input class="form-check-input" type="radio" name="is_pet"
												id="is_pickup" value="1"
												${accomodation.is_pickup == 1?"checked":""} disabled>
											<label class="form-check-label" for="is_pet">픽업가능</label>
										</div>
										<div class="col-3 form-check mx-3">
											<input class="form-check-input" type="radio"
												name="is_parking" id="is_parking" value="1"
												${accomodation.is_parking == 1?"checked":""} disabled>
											...
											<c:when test="${accomodation.is_deleted == 1}">
												<div class="col-6 mb-3">
													<button type="button"
														...
														onclick="confirmRestore(${accomodation.content_id})">복원</button>
												</div>
												<div class="col-6 mb-3">
													<button type="button"
														class="btn btn-outline-secondary w-100"
														onclick="location.href='../content/accomodation?currentPage=1'">취소</button>
												</div>
											</c:when>
											<c:when test="${accomodation.status == 0}">
												<div class="col-6 mb-3">
													<button type="button"
														class="form-control btn btn-primary w-100"
														onclick="approveConfirm()">승인(게시하기)</button>
												</div>
												<div class="col-3 mb-3">
													<button type="button"
														class="btn btn-outline-secondary w-100"
														data-bs-toggle="modal" data-bs-target="#rejectModal">반려(사유선택)</button>
												</div>
												<div class="col-3 mb-3">
													<button type="button"
														class="btn btn-outline-secondary w-100"
														onclick="location.href='../content/accomodation?currentPage=1'">목록</button>
												</div>
											</c:when>
											<c:when test="${accomodation.status == 1}">
												<div class="col-6 mb-3">
													<button type="button"
														...
														onclick="location.href='../content/accomodationUpdateForm?contentId=${accomodation.content_id}&currentPage=${currentPage}'">수정하기</button>
												</div>
												<div class="col-2 mb-3">
													<button type="button"
														class="btn btn-outline-secondary w-100"
														onclick="approveConfirm1()">반려전환</button>
												</div>
												<div class="col-2 mb-3">
													<button type="button"
														class="btn btn-outline-secondary w-100"
								...
								<form name="updateForm">
									<input type="hidden" name="status" id="statusInput"
										value="${accomodaiton.status}"> <input type="hidden"
										name="big_code" value="${accomodaiton.big_code}"> <input
										type="hidden" name="small_code"
										value="${accomodaiton.small_code}"> <input
										type="hidden" name="target_id"
										value="${accomodaiton.content_id}"> <input
										type="hidden" name="title" id="reject-title"> <input
										type="hidden" name="content" id="reject-content">
								</form>
							</div>
						</div>
					</div>
